...
# Stel je gebruikt de fetch_acc , dan staat het intel adress van de 6502 A-Register in %eax en die word gepushd 
# Stel je gebruikt != fetch_acc, dan staat het intel adress van de operand 
		
.text
	error: .asciz		"invalid instrucion \n" # error string for when an invalid instrucion is called
	debugstr: .asciz	"eax: %x\n"
...
decode:
	...
	#
	popl 	%ebx			# restore the EBX

	pushl 	%eax 			# push the return value on stack
	#pushl	MEM(%eax)		# push return value on stack
	addl	$4,	%ebx		# the execute method is in 2*IR + 1 (a long is 4)
	#incw	(PC)				# increment the Program Counter --> debug: eruit gehaald... overal in fetch methodes gezet

							#(return value of fetch-function is already on stack, no need for pushing parameter)
	call	*TABLE(%ebx)	# call execute method 	
							# no need to increment the PC: this is done in the fetchcycle				
...
# everything that has no addres mode , has none as a fetch routine
# this subroutine does nothing
none:
	ret
	
#This subroutine will terminate the program if an invalid instruction is called
invalid: 	
