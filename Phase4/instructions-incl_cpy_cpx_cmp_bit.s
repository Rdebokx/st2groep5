...
do_adc: 							# Decrement Y by one
	...
	je		adc_reset_x86carry		# reset x86 carry to false
	stc								# set x86 carry
...
adc_add:
	...
	movb	8(%ebp),%bl			# copy addressing to tempaddress

	adcb	(A),%bl					# add A and memory content with carry
	
	jnc		adc_carry_flag_off		# jmp if carry
	call	set_flag_carry			# set carry flag to false
	jmp		adc_overflow			# jump to next
	
adc_carry_flag_off:
	call	reset_flag_carry		# set carry flag to false
	
adc_overflow:
	jno		adc_overflow_flag_off	# jmp if x86 overflow not
	call	set_flag_overflow		# set zero flag to false
	jmp		adc_negative			# jump to next
...
adc_negative:
	...
	jge		adc_negative_flag_off	# A >= 0
	call	set_flag_negative		# set negative flag to true
	jmp		adc_zero				# jump to next

adc_negative_flag_off:
	call	reset_flag_negative		# set negative flag to false
	
adc_zero:
	cmp		$0,(A)					# compare 0 with A
	jne		adc_zero_flag_off		# A != 0
...
do_and: 						
	...
	movl	$0, %ebx			# clear ebx
	movl	MEM(%eax), %ebx		# move parameter to ebx
	andb	%bl, A				# AND on A register of C64
...
do_beq:
   ...
   # ************************************************************************
   # * Subroutine name 	: BIT (Bit test)						  	        *
   # * Description     	: The BIT instruction performs an "AND" on a memory	*
   # *				  	  and the value in the accumulator withoud affecting*
   # * 					  either, and sets the zero flag according to the 	*
   # *					  result. Furthermore, the negative flag is set 	*
   # * 					  equal to bit 7 of the memory value of the overflow*
   # * 					  flag is set equal to bit 6 of the memory value	*
   # * 					  tested.											*
   # * Author          	: Pascal											*
   # ************************************************************************
...
do_bit:
	...
	andb	%bl, %al			# Perform AND operation
	#movb	%al, %dl			# Move to %edx for further operations (bit 6 check)
	pushl	%eax				# Move to stack for further operations (bit 6 check)
	
	andb 	10000000, %al		# Check if bit 7 in %eax is positive (not sure on endianness)
	cmp		$0, %al				# Compare to 0..
	je		bit_negreset		# If 0, then reset negative flag.
	call 	set_flag_negative	# Else, set flag.
	jmp		bit_overflowcheck	# And go to next check.
	
bit_negreset:
	call 	reset_flag_negative
	
bit_overflowcheck:
	popl	%edx				# Pop the value off the stack for checking bit 6
	andb 	01000000, %dl		# Check if bit 6 in %edx is positive (not sure on endianness)
	cmp		$0, %dl				# Compare to 0..
	je		bit_ovfreset		# If 0, then reset negative flag.
	call 	set_flag_overflow	# Else, set flag.
	jmp		bit_end				# Go to end.
	
bit_ovfreset:
	call 	reset_flag_overflow
...
do_clv: 							# Clear overflow flag
   ...
   # ************************************************************************
   # * Subroutine name 	: CMP (Compare memory and accumulator)  	        *
   # * Description     	: The CMP instruction compares the value in the	   	*
   # *				  	  memory with the value in the accumulator.			*
   # * 					  It sets the carry flag if the memory value is 	*
   # *					  less than or equal to the accumulator value, and	*
   # * 					  reset if it is larger.							*
   # * Author          	: Roy												*
   # * Note				: The set_flag subroutines are also used by other	*
   # *    				  compare methods						   			*
   # ************************************************************************
do_cmp:
	pushl 	%ebp                    #   push base pointer
	...
	movl	MEM(%eax), %eax			# store the value of the memory in EAX
	movl	(A), %ebx				# store the value of the Acumulator in EBX
	subl	%eax, %ebx				# substract the value of the memory value from the acumulator. result is in EBX
	cmp		$0, %ebx				# test if the result is negative, equal or positive
	jge		do_cmp_set_carry		# if the restult is postive (greater than or equal to 0), the the memory value is smaller than or equal to the accumulator
	jmp		do_cmp_reset_carry		# if not, reset the carry
...
do_cpx:
	...
	movl	MEM(%eax), %eax			# store the value of the memory in EAX
	movl	(X), %ebx				# store the value of the Acumulator in EBX
	subl	%eax, %ebx				# substract the value of the memory value from the acumulator. result is in EBX
	cmp		$0, %ebx				# test if the result is negative, equal or positive
	jge		do_cmp_set_carry		# if the restult is postive (greater than or equal to 0), the the memory value is smaller than or equal to the accumulator
	jmp		do_cmp_reset_carry		# if not, reset the carry
...
do_cpy:
	...
	movl	MEM(%eax), %eax			# store the value of the memory in EAX
	movl	(Y), %ebx				# store the value of the Acumulator in EBX
	subl	%eax, %ebx				# substract the value of the memory value from the acumulator. result is in EBX
	cmp		$0, %ebx				# test if the result is negative, equal or positive
	jge		do_cmp_set_carry		# if the restult is postive (greater than or equal to 0), the the memory value is smaller than or equal to the accumulator
	jmp		do_cmp_reset_carry		# if not, reset the carry
...
do_dec: 							# Decrement Memory by one
	...
	movb    MEM(%eax), %bl 			#   Move the memory value to %ebx
	decb	%bl 					#   Decrease %ebx with one (the memory value)
	movb 	%bl, MEM(%eax) 			#   Move the decreased memory value to the real Memory 
...
do_inc: 							# Increment Memory by one
	...
	movl	$0, %ebx			#	Clear %ebx
	movb    MEM(%eax), %bl	 	#   Move the memory value to %ebx
	incb	%bl					#   Decrease %ebx with one (the memory value)
	movb 	%bl, MEM(%eax) 		#   Move the decreased memory value to the real Memory 
	movl 	$0, %eax 		# clear the %eax
	movb 	%bl, %al 		# Move the value of the bl to the al for flags
	call 	do_flag_zn 	 	# Set the negative and zero flags
	movl 	%ebp, %esp 				# copy stack pointer to ESP
	popl 	%ebp					# restore base pointer
	ret								# return from subroutine
...
do_lda:
	...
	movb	MEM(%eax), %al		# Get memory from adres %eax and put it in temp
...
do_lsr:
	...
	pushl 	%ebp                    #   push base pointer
	movl    %esp, %ebp              #   copy stack pointer
	movb 	MEM(%eax) , %bl	# move the value of the memory addres to the %ebx
...
do_lsr_func:
	...
	movl 	8(%ebp), %eax   # move the 	
	movb 	%bl, MEM(%eax)  # move the value of bl to the memory addres
...
do_ora:
	...
	movl	(A), %ebx				# store the value of the accumulator in EBX
	movl	MEM(%eax), %eax			# store the value of the place in the memory where the stackpointer points to, in EAX
...
do_pha:
	...
	movl	(A), %eax				# Store the value of the Acumulator in EAX
	movl	(S), %ebx				# Store the value of the Stackpointer in EBX
	movl	%eax, MEM(%ebx)			# Move the value of the acumulator to the stack
	decl	S						# Decrease the Stackpointer
...
do_pla:
	...
	movl	$0, %ebx				# Clear EBX
	movl	MEM(%eax), %ebx				# Store the value the stackpointer points to

	movb	%bl, A					# Move the value to the C64 P Register
	incl	S					# Increase the Stackpointer
...
do_plp:
	...
	incl	S						# Decrease the Stackpointer
...
do_rol:
	...
	movb 	MEM(%eax) , %bl 	# move the memory value to the %ebx
	call 	 isset_flag_carry 	# check if the carry flag is set , returns in %eax 
	cmp 	$0, %eax		# if there isn't a carry jump else just do ROL	
...
sbc_add:
	...
	movb	MEM(%eax),%bl			# copy memory content to temp
...
do_sta:
	...
	movb	%bl, MEM(%eax)		# move the bl register to the memory addres
...
do_tax:
  ...
  # ************************************************************************
   ...
   # * Description     	: The TAY instruction copies the value in the		*
